/* globals MozActivity, WebActivity */
...
let Frogger = (function() {
    ...
    if ('imageSmoothingEnabled' in drawingSurface) {
        drawingSurface.imageSmoothingEnabled = false;
        backgroundDrawingSurface.imageSmoothingEnabled = false;
    } else {
        drawingSurface.mozImageSmoothingEnabled = false;
        backgroundDrawingSurface.mozImageSmoothingEnabled = false;
    }

    drawingSurface.imageSmoothingQuality = 'low';
    backgroundDrawingSurface.imageSmoothingQuality = 'low';
...
(function(Frogger) {
    ...
    let _score = 0,
        ...
        _highScore = Number(localStorage.getItem('hs')) || 0,
        ...
        // Define a Boolean variable to indicate whether the player has moved yet
    ...
    // Define a function to execute when the player moves their character on the game board
    function playerMoved() {
        _hasPlayerMoved = true;
    }

    // When a new max row has been reached, increment the player's score
    ...
    Frogger.observer.subscribe("game-paused", function() {
        _isPlayerFrozen = true;
    });

    Frogger.observer.subscribe("game-unpaused", function() {
        if (_lives > 0) {
            _isPlayerFrozen = false;
        }
    });
    ...
    // When a new max row is reached, increment the player's score
    Frogger.observer.subscribe("new-top-row", onNewTopRow);
...
(function(Frogger) {
    ...
    function renderText(text, align, color, x, y) {
        ...
        Frogger.drawingSurface.fillText(text, x, y);
    }

    function renderCenterText(text) {
        renderText(text, "center", "#FFF", Frogger.drawingSurfaceWidth / 2, _gameBoard.rows[9]);
    }
    ...
    Frogger.observer.subscribe("high-score-change", function(newHighScore) {
        ...
        localStorage.setItem('hs', _highScore);
    });
...
(function(Frogger) {
    ...
    let _background = document.createElement("img");
...
(function(Frogger) {
    ...
    function renderTimeRemaining() {
        ...
        let rectangleWidth = _timeRemainingAsPercentage * _gameBoard.rows[10],
        ...
        let red = "#F00";
        let green = "#0F0";
        let timeBarColor = ((_timeRemainingAsPercentage < 0.2) ? red : ((_timeRemainingAsPercentage < 0.5) ? yellow : green));

        // Set the drawing context to draw in green, or red when it falls below 20%
        Frogger.drawingSurface.fillStyle = timeBarColor;
    ...
    Frogger.observer.subscribe("player-lost-life", function() {
        ...
        setTimeout(function() {
            Frogger.observer.publish("render-ad");
        });
    });
...
Frogger.Character = (function(Frogger) {
    ...
    let _character,
        ...
        // Define a variable to store the top row the player's character has reached
        ...
        // Define a Boolean variable to indicate whether the game is paused
        _isGamePaused = false,

        // Define a Boolean variable to indicate whether the game is over
        _isGameOver = false,

        // Define a Boolean variable to indicate whether an ad is visible
        _adVisible = false;
    ...
    function onCurrentRowChange() {
        let prevHighest = _highestRow;
        
        _highestRow = Math.min(_highestRow, _currentRow);

        if (_highestRow < prevHighest) {
            Frogger.observer.publish("new-top-row", _highestRow);
        }
    }
    ...
    Character.prototype.moveDown = function() {
        ...
        if (this.top > _gameBoard.characterBounds.bottom) {
            ...
            _currentRow++; // Keep track of the current row the character sits upon. Moved to else, to prevent "moving away" when moveDown from the bottom of the screen
            onCurrentRowChange();
        }
    ...
    function move(characterDirection) {
        ...
        if (!_isFrozen && !_isGamePaused) {
            ...
            // Publish an event to the rest of the code modules, indicating that the
            // player's position has been moved by the player in a certain direction
            if (direction !== null) {
                Frogger.observer.publish("player-moved-" + direction);
            }
        }
    }
    ...
    // Define a function to set the local _isGamePaused variable to true, indicating that the
    // game is paused by the user's request.
    function pause() {
        _isGamePaused = true;
    ...
    // Define a function to set the local _isGamePaused variable to false, indicating that the
    // game is unpaused by the user's request.
    function unpause() {
        _isGamePaused = false;
    ...
    Frogger.observer.subscribe("game-over", gameOver);
    ...
    Frogger.observer.subscribe("ad-shown", function() {
        _adVisible = true;
    });

    Frogger.observer.subscribe("ad-hidden", function() {
        _adVisible = false;
    });
    ...
    window.addEventListener("keydown", function(event) {
        ...
        // Skip key events when ads are shown
        if (_adVisible) {
            return true;
        }
        ...
        if (event.key === 'Enter' || event.key === 'Space') {
            event.preventDefault();
            if (_isGameOver) {
                // Reload app window to restart game
                location.reload();
            } else {
                if (_isGamePaused) {
                    unpause();
...
Frogger.Row = (function() {
    ...
    Row.prototype = {
        ...
        render: function(increment) {
            let index = 0,
                ...
                left,
                obstaclesItem,
                willIncrement = (increment === true || increment === false) ? increment : true;
            ...
            for (; index < length; index++) {
                ...
                let delta = ((this.direction === Frogger.direction.RIGHT ? 1 : -1) * this.speed);
                left = obstaclesItem.getPosition().left + ((willIncrement) ? delta : 0);
    ...
    Log.prototype.render = function(increment) {
        ...
        if (Frogger.Character.getTop() === this.getTop()) {
            let delta = ((this.direction === Frogger.direction.RIGHT ? 1 : -1) * this.speed);
            Frogger.Character.setPosition(Frogger.Character.getPosition().left + delta);
        }

        // Call the inherited render() method to draw the log in its new position
        Row.prototype.render.call(this, increment);
    ...
    Turtle.prototype.isCollision = function(characterPosition) {
        let isCollision = Log.prototype.isCollision.call(this, characterPosition);
...
// Support KaiOS-specific features and in-app ads
(function() {
    // KaiAds Info
    let PUBLISHER_ID = '';
    let AD_TIMEOUT = 45 * 1000; // 45s
    let APP_NAME = '';

    // Manifest URL to launch KaiStore
    let INTERSTITIAL_MANIFEST_URL = '';
    let INTERSTITIAL_RATE = 0.75; // 25%

    let screenLock, cpuLock;
    let preloadInterval = null;

    let interstitial = document.getElementById('app-interstitial');
    let softKeys = interstitial.querySelectorAll('.soft-keys > [data-action]');
    let kaiad = null;
    let adVisible = false;

    function onBeforeExit() {
        let c = confirm('Exit Frogr?');
        if (c) {
    ...
    window.addEventListener("keydown", function(event) {
        if (event.key === 'MicrophoneToggle') {
            event.preventDefault();
            return false;
        }

        if (!adVisible && !isInterstitialVisible()) {
        ...
        // Handle app interstitial key events
        if (isInterstitialVisible()) {
            let handled = onInterstitialSoftKeyClick(event.key);
            if (handled) {
                event.preventDefault();
                return false;
            }
    ...
    function isInterstitialVisible() {
        return !interstitial.hasAttribute('hidden');
    }

    function setInterstitialVisibility(visible) {
        if (visible) {
            interstitial.removeAttribute('aria-hidden');
            interstitial.removeAttribute('hidden');
        } else {
            interstitial.setAttribute('aria-hidden', 'true');
            interstitial.setAttribute('hidden', 'true');
            onAdClose();
        }
    }
    ...
    function onAdClose() {
        ...
        Frogger.observer.publish("ad-hidden");
        Frogger.observer.publish("game-unpaused");
    }

    function onKaiAdClose() {
        kaiad = null;
        adVisible = false;
        onAdClose();
    }

    function maybeRenderAd() {
        if (adVisible || isInterstitialVisible()) {
            return;
        }
      
        let renderInterstitial = (Math.random() > INTERSTITIAL_RATE);

        if (kaiad) {
            kaiad.on('close', onKaiAdClose);
            kaiad.on('click', onKaiAdClose);
      
            kaiad.call('display');
            adVisible = true;
            document.body.classList.add('ad-visible');
            Frogger.observer.publish("game-paused");
            Frogger.observer.publish("ad-shown");
            return true;
        } else if (renderInterstitial) {      
            setInterstitialVisibility(true);
            Frogger.observer.publish("game-paused");
            Frogger.observer.publish("ad-shown");
            return true;
        } 
    ...
    Frogger.observer.subscribe("render-ad", function() {
        setTimeout(maybeRenderAd);
    });

    // KaiOS 2.5
    let hasMozActivity = Boolean(
        typeof window === 'object' && 'MozActivity' in window
    );

    // KaiOS 3.0
    let hasWebActivity = Boolean(
        typeof window === 'object' && 'WebActivity' in window
    );

    function openAppPromise(url) {
        ...
        if (hasMozActivity) {
            ...
            // KaiOS 3.0
            let activity = new WebActivity('open-page', {
                type: 'url',
                url: url,
            });
    ...
    function onInterstitialSoftKeyClick(key) {
        switch (key) {
          case 'Backspace':
          case 'SoftLeft':
    ...
    // Bind to click events for soft keys
    for (let i = 0, e = softKeys.length; i < e; i++) {
        let button = softKeys[i];
        let key = button.dataset.key;
        button.addEventListener('click', function() { return onInterstitialSoftKeyClick(key); });
    }

    function preloadAds() {
        if (!navigator.onLine || kaiad !== null) {
            return;
        }
    ...
    // Preload KaiAd if connected to the internet
    preloadInterval = setInterval(preloadAds, AD_TIMEOUT);

    setTimeout(acquireWakeLock);
})();

// Now the code modules have been registered, kick off the game logic and start the game
Frogger.observer.publish("game-load");

// Call GC to reduce memory usage before game play begins
setTimeout(function() {
    try {
